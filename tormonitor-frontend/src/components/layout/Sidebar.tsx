...
export function Sidebar({ currentPage, onPageChange, onClose, isMobile = false, role }: SidebarProps) {
  ...
  return (
    <aside className={isMobile 
      ...
      {role === 'admin' ? (
        <div className="mt-8 mb-6 p-4 bg-forest/10 rounded-2xl border border-forest/20 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-16 h-16 bg-forest/20 rounded-full blur-xl -translate-y-1/2 translate-x-1/2"></div>
          <h4 className="font-display font-bold text-sm text-forest mb-1">Status Sistem</h4>
          <div className="flex items-center gap-2 mt-2">
            <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
            <span className="text-[11px] text-forest/80 font-bold">Semua Sistem Normal</span>
          </div>
          <p className="text-[10px] text-forest/50 mt-1">Database & API terhubung • v1.0.2</p>
        </div>
      ) : (
